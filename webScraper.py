import streamlit as st, re, ssl, json, time
...
# ---- 8 Globals ----
KEY="KEY"; CL=OpenAI(api_key=KEY); H={"User-Agent":"Mozilla/5.0"}; TR=2000
TMOUT=10; DELAY=0.5; FP="scrape"; MAXDEF=5
...
class Parser(HTMLParser):
    def __init__(s): super().__init__(); s.d={'links':[],'headings':[],'images':[],'text':[],'emails':[],'phones':[]}; s.t=None
    def handle_starttag(s,t,a):
        a=dict(a)
        if t=='a' and 'href' in a: s.d['links'].append(a['href'])
        if t=='img' and 'src' in a: s.d['images'].append({'src':a['src'],'alt':a.get('alt','')})
        if t in ('h1','h2','h3','h4'): s.t=t
    def handle_endtag(s,t):
        if t in ('h1','h2','h3','h4'): s.t=None
    def handle_data(s,x):
        x=x.strip()
        if not x: return
        if s.t: s.d['headings'].append({'level':s.t,'text':x})
        s.d['text'].append(x)
...
def norm(u, b):
    if not u or u.startswith(('#', 'js', 'mail', 'tel')): return None
    f = urljoin(b, u); p = urlparse(f)
...
def crawl(start, maxp, cb):
    ...
    while to_do and len(vis) < maxp:
        ...
        try:
            ...
            for l in p.d['links']:
                n = norm(l, u)
                if n and urlparse(n).netloc == dom and n not in vis: to_do.append(n)
            vis.add(u); time.sleep(DELAY)
        except Exception as e:
            res[u] = {'status': 'fail', 'err': str(e)}; vis.add(u)
    return res, vis
...
def aggr(r):
    ...
    for v in r.values():
        if v.get('status') == 'ok':
            for k in d: d[k].extend(v['data'][k])
    return d
...
def stats(d):
    ...
    return {'tw': len(w), 'uw': len(set(w)), 'top': Counter(f).most_common(15),
...
def save(url, r, a, s):
    ...
    with open(fn, 'w', encoding='utf-8') as f: f.write(json.dumps(out, indent=2))
    return fn
...
if st.button("🚀 Crawl"):
    if not url:
        ...
        c1.metric("Words", f"{s['tw']:,}"); c2.metric("Unique", f"{s['uw']:,}")
        c3.metric("Emails", s['em']); c4.metric("Phones", s['ph'])
        ...
        with st.expander("📧 Emails"): st.write(list(set(a['emails'])) or "None")
        with st.expander("📱 Phones"): st.write(list(set(a['phones'])) or "None")
        st.header("🗂️ Pages")
        for u, d in r.items():
            if d.get('status') == 'ok':
                st.markdown(f"✅ **{u}** — {d['links']} links, {d['images']} imgs, {d['headings']} headings")
                with st.expander(f"🔗 Links in {u}"): st.write(d['data']['links'] or "None")
                with st.expander(f"📑 Headings in {u}"): st.write(d['data']['headings'] or "None")
            else:
